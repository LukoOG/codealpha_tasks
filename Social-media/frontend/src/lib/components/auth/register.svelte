<script lang="ts">
	import Input from '$lib/components/ui/input.svelte';
	import Button from '$lib/components/ui/button.svelte';
	import { Label } from 'bits-ui';
	import { EyeOff, Eye, Check } from '@lucide/svelte';

	let { disable } = $props();

	let showPassword1 = $state<boolean>(false);
	let showPassword2 = $state<boolean>(false);
	let password1 = $state<string>('');
	let password2 = $state<string>('');
	let agreeToTerms = $state(false);

	// Django's default password validation:
	// - At least 8 characters
	// - Not entirely numeric
	let passwordTooShort = $derived(password1.length > 0 && password1.length < 8);
	let passwordAllNumeric = $derived(password1.length > 0 && /^\d+$/.test(password1));
	let passwordValid = $derived(password1.length >= 8 && !/^\d+$/.test(password1));
	let passwordMatch = $derived(
		password1.length > 0 && password2.length > 0 && password1 === password2
	);
	let passwordMismatch = $derived(password2.length > 0 && password1 !== password2);

	let disableSubmit = $derived(!agreeToTerms || !passwordMatch || !passwordValid);
	$inspect(password2);
</script>

<div class="space-y-2">
	<Label.Root htmlFor="username">Username</Label.Root>
	<Input id="username" type="text" placeholder="Choose a username" required />
</div>

<div class="space-y-2">
	<Label.Root htmlFor="register-email">Email</Label.Root>
	<Input id="register-email" type="email" placeholder="Enter your email" required />
</div>

<div class="space-y-2">
	<Label.Root htmlFor="register-password">Password</Label.Root>
	<div class="relative">
		<Input
			id="register-password"
			type={showPassword1 ? 'text' : 'password'}
			placeholder="Create a password"
			bind:value={password1}
			className={passwordTooShort || passwordAllNumeric ? 'border-destructive' : ''}
			required
		/>
		<button
			type="button"
			class="absolute top-1/2 right-3 -translate-y-1/2 text-muted-foreground hover:text-foreground"
		>
			{#if showPassword1}
				<EyeOff onclick={() => (showPassword1 = false)} size={16} />
			{:else}
				<Eye onclick={() => (showPassword1 = true)} size={16} />
			{/if}
		</button>
	</div>
	{#if passwordTooShort}
		<p class="text-xs text-destructive">Password must be at least 8 characters.</p>
	{/if}
	{#if passwordAllNumeric}
		<p class="text-xs text-destructive">Password can't be entirely numeric.</p>
	{/if}
</div>

<div class="space-y-2">
	<Label.Root htmlFor="confirm-password">Confirm Password</Label.Root>
	<div class="relative">
		<Input
			id="confirm-password"
			type={showPassword2 ? 'text' : 'password'}
			placeholder="Confirm your password"
			bind:value={password2}
			className={passwordMismatch ? 'border-destructive' : ''}
			required
		/>
		<button
			type="button"
			class="absolute top-1/2 right-3 -translate-y-1/2 text-muted-foreground hover:text-foreground"
		>
			{#if showPassword2}
				<EyeOff onclick={() => (showPassword2 = false)} size={16} />
			{:else}
				<Eye onclick={() => (showPassword2 = true)} size={16} />
			{/if}
		</button>
		{#if passwordMatch}
			<div class="absolute top-1/2 right-10 -translate-y-1/2 text-social-green">
				<Check size={16} />
			</div>
		{/if}
	</div>
	{#if passwordMismatch}
		<p class="text-xs text-destructive">Passwords do not match.</p>
	{/if}
</div>

<div class="space-y-3">
	<label class="flex items-start space-x-2 text-sm">
		<input
			type="checkbox"
			class="mt-0.5 rounded border-border"
			required
			bind:checked={agreeToTerms}
		/>
		<span class="text-muted-foreground">
			I agree to the{' '}
			<button type="button" class="text-primary hover:underline"> Terms of Service </button>{' '}
			and{' '}
			<button type="button" class="text-primary hover:underline"> Privacy Policy </button>
		</span>
	</label>
</div>

<Button type="submit" disabled={disableSubmit} class="w-full">Create Account</Button>

<div class="relative">
	<div class="absolute inset-0 flex items-center">
		<span class="w-full border-t border-border"></span>
	</div>

	<div class="relative flex justify-center text-xs uppercase">
		<span class="bg-card px-2 text-muted-foreground">Or continue with</span>
	</div>
</div>

<div class="grid grid-cols-2 gap-2">
	<Button variant="outline" type="button" class="w-full">
		<svg class="mr-2 h-4 w-4" viewBox="0 0 24 24">
			<path
				fill="currentColor"
				d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
			/>
			<path
				fill="currentColor"
				d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
			/>
			<path
				fill="currentColor"
				d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
			/>
			<path
				fill="currentColor"
				d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
			/>
		</svg>
		Google
	</Button>

	<Button variant="outline" type="button" class="w-full">
		<svg class="mr-2 h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
			<path
				d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"
			/>
		</svg>
		Facebook
	</Button>
</div>
